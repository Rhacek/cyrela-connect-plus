...
export function LeadCard({ lead, showActions = true, className }: LeadCardProps) {
  ...
  return (
    <div className={cn(
      "bg-white rounded-lg p-4 shadow-sm border border-cyrela-gray-lighter flex flex-col",
      lead.isManual && "border-l-4 border-l-cyrela-blue",
      className
    )}>
      <div className="flex justify-between items-start">
        <div className="flex items-center min-w-0 flex-1">
          <div className="w-10 h-10 rounded-full bg-cyrela-gray-lighter flex items-center justify-center shrink-0">
        ...
        <Badge className={cn(
          "px-2 py-1 font-normal whitespace-nowrap ml-2 shrink-0",
      ...
      <div className="mt-3 space-y-2">
        <div className="flex items-center text-sm">
          <Phone size={16} className="mr-2 text-cyrela-gray-dark shrink-0" />
          <span className="truncate">{lead.phone}</span>
        </div>
        
        <div className="flex items-center text-sm">
          <Mail size={16} className="mr-2 text-cyrela-gray-dark shrink-0" />
          <span className="truncate">{lead.email}</span>
        </div>
        
        <div className="flex items-center text-sm flex-wrap">
          <div className="flex items-center mr-4 mb-1">
            <Calendar size={16} className="mr-2 text-cyrela-gray-dark shrink-0" />
            <span>{formatDate(lead.createdAt)}</span>
          </div>
          <div className="flex items-center">
            <Clock size={16} className="mr-2 text-cyrela-gray-dark shrink-0" />
      ...
      {showActions && (
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <Button className="ml-auto bg-cyrela-blue hover:bg-cyrela-blue hover:opacity-90 text-white shrink-0" size="sm">
            Atualizar status
          </Button>
        </div>
      )}
